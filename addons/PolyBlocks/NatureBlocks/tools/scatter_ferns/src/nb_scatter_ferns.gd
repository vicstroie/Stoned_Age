# POLYBLOCKS | NATUREBLOCKS | FERN SCATTER TOOL
# bukkbeek.github.io
...
@export_group("Ferns")
@export_range(1, 100000, 1) var fern_amount: int = 50
@export_range(0.01, 10.0, 0.01) var fern_scale_min: float = 0.8
@export_range(0.01, 10.0, 0.01) var fern_scale_max: float = 1.2
@export var fern_surface_offset: float = -0.02

@export_group("Vegetation")
@export_range(0.0, 100.0, 0.1) var forest: float = 33.3
@export_range(0.0, 100.0, 0.1) var jungle: float = 33.3
@export_range(0.0, 100.0, 0.1) var olive: float = 33.4

@export_group("Resource Paths")
@export_file var fern_mesh_path: String = "res://PolyBlocks/NatureBlocks/tools/scatter_ferns/src/nbmesh_fern1.mesh"
@export_dir var fern_materials_root_path: String = "res://PolyBlocks/NatureBlocks/source_files/materials/ferns"

@export_group("Generated Node")
@export var generated_ferns_name: String = "nbgen_ferns"
...
func generate() -> void:
	if surface_mesh == null:
		push_warning("FernScatterTool: no surface_mesh assigned.")
		return

	if surface_mesh.mesh == null:
		push_warning("FernScatterTool: surface_mesh has no Mesh resource.")
		return

	if fern_scale_min > fern_scale_max:
		push_warning("FernScatterTool: fern_scale_min is greater than fern_scale_max.")
		return
	...
	_remove_generated_ferns()

	var triangle_data: Dictionary = _build_triangle_distribution(surface_mesh.mesh)

	if triangle_data.is_empty():
		push_warning("FernScatterTool: no usable top-facing surface area found.")
		return

	_scatter_ferns(triangle_data)
...
func _build_triangle_distribution(mesh: Mesh) -> Dictionary:
	...
	if triangles.is_empty():
		push_warning("FernScatterTool: no top-facing triangles found.")
		return {}
	...
	if cumulative_areas.is_empty() or total_area <= 0.0:
		push_warning("FernScatterTool: no valid triangle area found.")
		return {}
...
func _scatter_ferns(triangle_data: Dictionary) -> void:
	if fern_scale_min > fern_scale_max:
		push_warning("FernScatterTool: fern_scale_min is greater than fern_scale_max.")
		return

	var weights: Dictionary = {
		"forest": clamp(forest, 0.0, 100.0),
		"jungle": clamp(jungle, 0.0, 100.0),
		"olive": clamp(olive, 0.0, 100.0),
	}

	var total_weight: float = weights["forest"] + weights["jungle"] + weights["olive"]

	if total_weight <= 0.0:
		push_warning("FernScatterTool: all vegetation percentages are zero; no ferns placed.")
		return

	var variants: Array[String] = ["forest", "jungle", "olive"]
	...
	var fern_mesh: Mesh = _load_fern_mesh()
	if fern_mesh == null:
		return

	var ferns_container := Node3D.new()
	ferns_container.name = generated_ferns_name
	surface_mesh.add_child(ferns_container)
	_set_editor_owner(ferns_container)
	...
	for v in variants:
		...
		var material: Material = _load_fern_material(v)
		if material == null:
			continue
		...
		mmi.name = "%s_%s" % [generated_ferns_name, v]
		mmi.multimesh = mm
		mmi.material_override = material

		ferns_container.add_child(mmi)
	...
	print(
		"FernScatterTool: scattered %d ferns across %d material(s) on %s."
		% [
			total_placed,
			variants_placed,
			surface_mesh.name
...
func _get_top_facing_triangles(mesh: Mesh) -> Array:
	...
	for surface_idx in mesh.get_surface_count():
		...
		if verts_v == null or normals_v == null:
			push_warning(
				"FernScatterTool: surface %d has no vertex/normal data, skipping."
				% surface_idx
			)
			continue
...
func _load_fern_mesh() -> Mesh:
	var mesh: Mesh = load(fern_mesh_path)
	if mesh == null:
		push_warning(
			"FernScatterTool: could not load fern mesh at %s"
			% fern_mesh_path
		)
		return null
...
func _load_fern_material(variant: String) -> Material:
	var mat_path: String = (
		"%s/nbmat_fern1_%s.tres"
		% [
			fern_materials_root_path,
			variant
		]
	)
	...
	if material == null:
		push_warning(
			"FernScatterTool: could not load fern material at %s"
			% mat_path
		)
...
func _remove_generated_ferns() -> void:
	if surface_mesh == null:
		return

	var existing: Node = surface_mesh.get_node_or_null(
		generated_ferns_name
	)
