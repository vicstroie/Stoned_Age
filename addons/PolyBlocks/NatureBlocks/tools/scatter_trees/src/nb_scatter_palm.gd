# POLYBLOCKS | NATUREBLOCKS | TREE [PALM] SCATTER TOOL
# bukkbeek.github.io
...
@export_dir var trees_root_path: String = "res://PolyBlocks/NatureBlocks/assets/trees/palm"

const GENERATED_META := "flat_scatter_generated"
const PALM_NODE_NAME := "palm"
...
func _scatter() -> void:
	if surface_mesh == null or surface_mesh.mesh == null:
		push_warning("PalmScatter: no target mesh assigned")
		return
	...
	print("PalmScatter: aabb=%s, top-facing triangles=%d" % [aabb, triangles.size()])

	if triangles.is_empty():
		push_warning("PalmScatter: no triangles on target mesh meet top_facing_threshold")
		return

	var tree_scenes := _get_tree_scenes()

	if tree_scenes.is_empty():
		push_warning("PalmScatter: no .scn/.tscn tree scenes found in %s" % trees_root_path)
		return
	...
	print("PalmScatter: candidates=%d, cluster=%.2f" % [points.size(), cluster])
	...
	print("PalmScatter: placed=%d, raycast misses=%d" % [placed, misses])
...
func _get_tree_scenes() -> Array[String]:
	...
	if dir == null:
		push_warning("PalmScatter: could not open tree directory: %s" % trees_root_path)
		return scenes
...
func _get_top_facing_triangles(mesh: Mesh) -> Array:
	...
	for surface_idx in mesh.get_surface_count():
		...
		if verts_v == null or normals_v == null:
			push_warning("PalmScatter: surface %d has no vertex/normal data, skipping" % surface_idx)
			continue
...
func _place_tree_at(local_pos: Vector3, surface_normal: Vector3, tree_scenes: Array[String]) -> bool:
	...
	if biome.is_empty():
		push_warning("PalmScatter: all biome weights are zero, skipping tree placement")
		return false
	...
	if matching_scenes.is_empty():
		push_warning("PalmScatter: no tree scene found for biome '%s'" % biome)
		return false
	...
	var palm_node: Node3D = surface_mesh.get_node_or_null(PALM_NODE_NAME)

	if palm_node == null:
		palm_node = Node3D.new()
		palm_node.name = PALM_NODE_NAME
		surface_mesh.add_child(palm_node)
		if Engine.is_editor_hint():
			var root := get_tree().edited_scene_root
			if root:
				palm_node.owner = root

	palm_node.add_child(instance)
...
func clear_trees() -> void:
	if surface_mesh == null:
		return
	var palm_node: Node = surface_mesh.get_node_or_null(PALM_NODE_NAME)
	if palm_node == null:
		return
	palm_node.free()
