# POLYBLOCKS | NATUREBLOCKS | TREE [WILLOW] SCATTER TOOL
# bukkbeek.github.io
...
@export_dir var trees_root_path: String = "res://PolyBlocks/NatureBlocks/assets/trees/willow"

const GENERATED_META := "flat_scatter_generated"
const WILLOW_NODE_NAME := "willow"
...
func _scatter() -> void:
	if surface_mesh == null or surface_mesh.mesh == null:
		push_warning("WillowScatter: no target mesh assigned")
		return
	...
	print("WillowScatter: aabb=%s, top-facing triangles=%d" % [aabb, triangles.size()])

	if triangles.is_empty():
		push_warning("WillowScatter: no triangles on target mesh meet top_facing_threshold")
		return

	var tree_scenes := _get_tree_scenes()

	if tree_scenes.is_empty():
		push_warning("WillowScatter: no .scn/.tscn tree scenes found in %s" % trees_root_path)
		return
	...
	print("WillowScatter: candidates=%d, cluster=%.2f" % [points.size(), cluster])
	...
	print("WillowScatter: placed=%d, raycast misses=%d" % [placed, misses])
...
func _get_tree_scenes() -> Array[String]:
	...
	if dir == null:
		push_warning("WillowScatter: could not open tree directory: %s" % trees_root_path)
		return scenes
...
func _get_top_facing_triangles(mesh: Mesh) -> Array:
	...
	for surface_idx in mesh.get_surface_count():
		...
		if verts_v == null or normals_v == null:
			push_warning("WillowScatter: surface %d has no vertex/normal data, skipping" % surface_idx)
			continue
...
func _place_tree_at(local_pos: Vector3, surface_normal: Vector3, tree_scenes: Array[String]) -> bool:
	...
	if biome.is_empty():
		push_warning("WillowScatter: all biome weights are zero, skipping tree placement")
		return false
	...
	if matching_scenes.is_empty():
		push_warning("WillowScatter: no tree scene found for biome '%s'" % biome)
		return false
	...
	var willow_node: Node3D = surface_mesh.get_node_or_null(WILLOW_NODE_NAME)

	if willow_node == null:
		willow_node = Node3D.new()
		willow_node.name = WILLOW_NODE_NAME
		surface_mesh.add_child(willow_node)
		if Engine.is_editor_hint():
			var root := get_tree().edited_scene_root
			if root:
				willow_node.owner = root

	willow_node.add_child(instance)
...
func clear_trees() -> void:
	if surface_mesh == null:
		return
	var willow_node: Node = surface_mesh.get_node_or_null(WILLOW_NODE_NAME)
	if willow_node == null:
		return
	willow_node.free()
