# POLYBLOCKS | NATUREBLOCKS | TREE [PINE] SCATTER TOOL
# bukkbeek.github.io
...
@export_range(0.0, 100.0, 1.0) var trunk: float = 100.0
@export_range(0.0, 100.0, 1.0) var low_poly: float = 100.0

@export_group("Resource Paths")
@export_dir var trees_root_path: String = "res://PolyBlocks/NatureBlocks/assets/trees/pine"

const GENERATED_META := "flat_scatter_generated"
const PINE_NODE_NAME := "pine"
...
func _scatter() -> void:
	if surface_mesh == null or surface_mesh.mesh == null:
		push_warning("PineScatter: no target mesh assigned")
		return
	...
	print("PineScatter: aabb=%s, top-facing triangles=%d" % [aabb, triangles.size()])

	if triangles.is_empty():
		push_warning("PineScatter: no triangles on target mesh meet top_facing_threshold")
		return

	var tree_scenes := _get_tree_scenes()

	if tree_scenes.is_empty():
		push_warning("PineScatter: no .scn/.tscn tree scenes found in %s" % trees_root_path)
		return
	...
	print("PineScatter: candidates=%d, cluster=%.2f" % [points.size(), cluster])
	...
	print("PineScatter: placed=%d, raycast misses=%d" % [placed, misses])
...
func _get_tree_scenes() -> Array[String]:
	return [
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine1_anime.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine1_blue.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine1_forest.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine1_jungle.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine1_olive.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine1_spooky.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine1_trunk.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine2_anime.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine2_blue.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine2_forest.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine2_jungle.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine2_olive.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine2_spooky.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine2_trunk.tscn",
		"res://PolyBlocks/NatureBlocks/assets/trees/pine/nb_pine3_lowpoly.tscn"
	]
...
func _get_top_facing_triangles(mesh: Mesh) -> Array:
	...
	for surface_idx in mesh.get_surface_count():
		...
		if verts_v == null or normals_v == null:
			push_warning("PineScatter: surface %d has no vertex/normal data, skipping" % surface_idx)
			continue
...
func _place_tree_at(local_pos: Vector3, surface_normal: Vector3, tree_scenes: Array[String]) -> bool:
	...
	if biome.is_empty():
		push_warning("PineScatter: all biome weights are zero, skipping tree placement")
		return false

	var biome_suffix: String
	match biome:
		"low_poly": biome_suffix = "lowpoly"
		_: biome_suffix = biome

	var matching_scenes: Array[String] = []
	for scene_path in tree_scenes:
		if scene_path.get_file().get_basename().ends_with("_" + biome_suffix):
			matching_scenes.append(scene_path)

	if matching_scenes.is_empty():
		push_warning("PineScatter: no tree scene found for biome '%s'" % biome)
		return false
	...
	var pine_node: Node3D = surface_mesh.get_node_or_null(PINE_NODE_NAME)

	if pine_node == null:
		pine_node = Node3D.new()
		pine_node.name = PINE_NODE_NAME
		surface_mesh.add_child(pine_node)
		if Engine.is_editor_hint():
			var root := get_tree().edited_scene_root
			if root:
				pine_node.owner = root

	pine_node.add_child(instance)
...
func _biome_weights() -> Dictionary:
	return {
		"forest": forest, "jungle": jungle, "olive": olive,
		"blue": blue, "anime": anime, "spooky": spooky,
		"trunk": trunk, "low_poly": low_poly
	}
...
func clear_trees() -> void:
	if surface_mesh == null:
		return
	var pine_node: Node = surface_mesh.get_node_or_null(PINE_NODE_NAME)
	if pine_node == null:
		return
	pine_node.free()
