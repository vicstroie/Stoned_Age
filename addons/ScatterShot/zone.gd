...
func global_transform_offset() -> Transform3D:
	if Engine.is_editor_hint():
		return Transform3D.IDENTITY
	return global_transform.inverse() * _initial_global_transform
...
func accumulate_jobs(layer: ScatterShotLayer, camera_pos: Vector3, camera_frustum: Array[Plane], grow_jobs: Dictionary[ScatterShotLayer, ChunkJob], prune_jobs: Dictionary[ScatterShotLayer, ChunkJob]) -> void:
	...
	var offset: Transform3D = global_transform_offset()
	var inverse_offset: Transform3D = offset.inverse()
	var initial_global_transform: Transform3D = global_transform * offset
	var inverse_global_basis: Basis = initial_global_transform.basis.inverse()
	var center3d: Vector3 = inverse_global_basis * initial_global_transform.origin
	...
	var target3d: Vector3 = inverse_global_basis * (offset * camera_pos)
	...
	while y < chunk_rect.end.y:
		...
		while x < chunk_rect.end.x:
			...
			if chunks.has(chunk_origin):
				...
				continue
			var chunk_pos_2d: Vector2 = (Vector2(chunk_origin) + Vector2(0.5 * layer.chunk_size, 0.5 * layer.chunk_size)) * layer.grid_scale
			var chunk_pos_3d: Vector3 = inverse_offset * (initial_global_transform.basis * Vector3(chunk_pos_2d.x, target3d.y, chunk_pos_2d.y))
...
func add_chunk(layer: ScatterShotLayer, chunk: ScatterShotChunk, origin: Vector2i) -> void:
	...
	var raycast_target_position_offset: Vector3 = global_transform.basis * Vector3(0, -depth, 0)
	chunk.items_begin(self)
	var y: int = 0
	while y < layer.chunk_size:
		...
		while x < layer.chunk_size:
			...
			var global: Vector3 = inverse_offset * (initial_global_transform.basis * Vector3(grid_space.x, center3d.y, grid_space.y))
